...
.PHONY: help install install-playwright install-dev test test-verbose test-coverage run clean docker-build docker-pull lint typecheck format
...
help:
	...
	@echo "  $(YELLOW)make install-dev$(NC)           Install dev dependencies (testing, linting)"
	@echo ""
	...
	@echo "  $(YELLOW)make lint$(NC)                  Run ruff linter"
	@echo "  $(YELLOW)make typecheck$(NC)            Run mypy type checker"
	@echo "  $(YELLOW)make format$(NC)                Run ruff and black formatters"
	@echo ""
...
# Run formatters
format:
	@echo "$(BLUE)Running ruff and black formatters...$(NC)"
	ruff format python/
	black python/
