...
- **Details:**
  - Added `healthcheck` block with test, interval, timeout, retries, and start_period
  - Mirrors the HEALTHCHECK directive already present in `docker/run/Dockerfile`
  - Uses curl to verify http://localhost:80/ is responding
- **Rationale:** The Dockerfile already has HEALTHCHECK (added in previous platform-engineer work), but docker-compose.yml was not utilizing it. Adding healthcheck to docker-compose enables proper container orchestration, allows dependent services to use `depends_on` with `condition: service_healthy`, and improves container reliability.
- **Impact:**
  - Docker Compose can monitor container health status
  - Services depending on agent-zero can wait for healthy state
  - Better startup sequencing for multi-container deployments
