...
- Issue #649: Fixed corrupt content in user-story-engineer.md
- Closed PR #644: Dead code removal had scope creep (626 files vs claimed 1)
...
- ~~`rate_limiter.py` (66 lines)~~ ✅ COMPLETED (PR #393/PR #443)
- ~~`errors.py` (66 lines)~~ ✅ COMPLETED
...
2. ~~**Add test for rate_limiter.py**~~ ✅ COMPLETED (PR #393/PR #443)
...
10. ~~**Add test for messages.py**~~ ✅ COMPLETED (20 tests)
11. ~~**Add test for tool.py**~~ ✅ COMPLETED
12. ~~**Add test for shell_local.py**~~ ✅ COMPLETED
...
## New Scan Findings (2026-03-03)

### Python Dead Code

1. **perplexity_search.py** (37 lines) - Unused module with commented-out code
   - Entire module appears unused (replaced by searxng)
   - Lines 18-25: Commented-out system prompt block

2. **faiss_monkey_patch.py** (37 lines) - Commented-out legacy code
   - Lines 1-11: Commented-out legacy imports/documentation
   - Module IS used (critical for Python 3.12 ARM)

### JavaScript Dead Code

1. **full-screen-store.js** - Empty `init()` function (lines 16-18)
   - No-op function kept for parity
   
2. **tasks-store.js** - Potential merge conflict markers (needs review)
   
3. **preferences-store.js** - Empty `_applyAutoSpeech()` function (lines 100-102)

4. **messages.js** - Commented-out code (lines 903-906)
...
- PRs must be up to date with default branch before merging
- Small atomic diff: 1-5 files max
