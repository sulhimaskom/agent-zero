...
SPECIAL_COMMANDS = {
    ...
    "find": {
        "max_args": 20,
        "allow_flags": ["-name", "-type", "-path", "-maxdepth", "-mindepth"],
    },
    "grep": {
        "max_args": 20,
        "allow_flags": ["-i", "-v", "-r", "-n", "-l", "-w", "-E", "-F"],
    },
}
...
def validate_command_structure(command: str) -> Tuple[bool, str]:
    ...
    if base_cmd in SPECIAL_COMMANDS:
        ...
        if len(args) > special_config["max_args"]:
            return (
                False,
                f"Too many arguments for command '{base_cmd}'. Maximum allowed: {special_config['max_args']}",
            )
