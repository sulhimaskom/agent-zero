...
class TaskSchedule(BaseModel):
    ...
    weekday: str
    timezone: str = Field(
        default_factory=lambda: Localization.get().get_timezone()
    )
...
class BaseTask(BaseModel):
    ...
    def check_schedule(
        self, frequency_seconds: float = Timeouts.SCHEDULER_CHECK_FREQUENCY
    ) -> bool:
        return False
    ...
    def get_next_run_minutes(self) -> int | None:
        ...
        if next_run is None:
            return None
        return int(
            (next_run - datetime.now(timezone.utc)).total_seconds() / 60
        )
...
class SchedulerTaskList(BaseModel):
    tasks: list[
        Annotated[
            Union[ScheduledTask, AdHocTask, PlannedTask],
            Field(discriminator="type"),
        ]
    ] = Field(default_factory=list)
    # Singleton instance
    __instance: ClassVar[Optional["SchedulerTaskList"]] = PrivateAttr(
        default=None
    )
            ...
            for task in self.tasks:
                if isinstance(task, AdHocTask):
                    if task.token is None or task.token == "":
                        PrintStyle(
                            italic=True, font_color="red", padding=False
                        ).print(
                        ...
                        task.token = str(
                            random.randint(
                                Limits.SCHEDULER_TOKEN_MIN,
                                Limits.SCHEDULER_TOKEN_MAX,
                            )
                        )
                        PrintStyle(
                            italic=True, font_color="red", padding=False
        ...
        with self._lock:
            ...
            return [
                ...
                and (
                    ...
                    or (now - task.last_run).total_seconds() >= 60
                )
            ]
    ...
    def get_task_by_uuid(
        ...
        with self._lock:
            return next(
                (task for task in self.tasks if task.uuid == task_uuid), None
            )
    ...
    def get_task_by_name(
        ...
        with self._lock:
            return next(
                (task for task in self.tasks if task.name == name), None
            )
    ...
    async def remove_task_by_uuid(self, task_uuid: str) -> "SchedulerTaskList":
        with self._lock:
            self.tasks = [
                task for task in self.tasks if task.uuid != task_uuid
            ]
            await self.save()
        return self
...
class TaskScheduler:
    ...
    async def run_task_by_name(
        self, name: str, task_context: str | None = None
    ):
    ...
    async def __new_context(
        ...
        context: AgentContext = AgentContext(
            config, id=task.context_id, name=task.name
        )
    ...
    async def _get_chat_context(
        ...
        context = (
            AgentContext.get(task.context_id) if task.context_id else None
        )
    ...
    async def _run_task(
        ...
        async def _run_task_wrapper(
            task_uuid: str, task_context: str | None = None
        ):
            ...
            if task_snapshot is None:
                self._printer.print(
                    f"Scheduler Task with UUID '{task_uuid}' not found"
                )
                return
            ...
            try:
                self._printer.print(
                    f"Scheduler Task '{current_task.name}' started"
                )
                ...
                # Error
                self._printer.print(
                    f"Scheduler Task '{current_task.name}' failed: {e}"
                )
