...
def handle_error_gracefully(
    error: Exception, context: str = "", log: Optional[Log.Log] = None
):
    """
    ...
    async def safe_wrapper():
        try:
            ...
            handle_error_gracefully(
                e, f"background task {name}" if name else "background task"
            )
...
def validate_required_params(
    params: dict, required_keys: list, context: str = ""
) -> None:
    """
    ...
    if missing_keys:
        ...
        raise ValueError(
            f"Missing required parameters{context_msg}: {', '.join(missing_keys)}"
        )
