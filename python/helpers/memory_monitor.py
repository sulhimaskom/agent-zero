...
class MemoryMonitor:
    ...
    def __init__(
        self, memory_threshold_percent: float = 80.0, check_interval: int = 60
    ):
        """
        ...
        while self._monitoring:
            try:
                ...
                if memory_percent > self.memory_threshold_percent:
                    PrintStyle.warning(
                        f"Memory usage ({memory_percent:.1f}%) exceeds threshold ({self.memory_threshold_percent}%)"
                    )
                    ...
                    if new_memory_percent > self.memory_threshold_percent:
                        PrintStyle.error(
                            f"Memory still high after cleanup: {new_memory_percent:.1f}%"
                        )
